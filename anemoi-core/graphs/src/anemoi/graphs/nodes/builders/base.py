...
class BaseNodeBuilder(ABC):
    ...
    def __init__(self, name: str) -> None:
        ...
        print(f"Initialized {self.__class__.__name__} with device: {self.device}")
    ...
    def register_nodes(self, graph: HeteroData) -> HeteroData:
        ...
        """
        print(f"Registering nodes for {self.__class__.__name__} on device: {self.device}")
        graph[self.name].x = self.get_coordinates().to(dtype=torch.float32)
